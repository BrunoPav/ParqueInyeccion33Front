import 'dart:convert';

import 'package:http/http.dart' as http;

import '../../../core/red/cliente_http.dart';
import 'servicio_dto.dart';

class FuenteRemotaServicios {
  static const String _ruta = '/api/servicios';

  Future<List<ServicioDto>> listar({int? vehiculoId}) {
    return ejecutar(() async {
      final parametros = <String, String>{};
      if (vehiculoId != null) {
        parametros['vehiculoId'] = vehiculoId.toString();
      }

      final respuesta = await http.get(construirUri(_ruta, parametros));
      final lista = decodificar(respuesta) as List<dynamic>;
      return lista
          .map((json) => ServicioDto.desdeJson(json as Map<String, dynamic>))
          .toList();
    });
  }

  Future<ServicioDto> obtener(int id) {
    return ejecutar(() async {
      final respuesta = await http.get(construirUri('$_ruta/$id'));
      return ServicioDto.desdeJson(decodificar(respuesta) as Map<String, dynamic>);
    });
  }

  Future<ServicioDto> crear(ServicioDto servicio) {
    return ejecutar(() async {
      final respuesta = await http.post(
        construirUri(_ruta),
        headers: cabecerasJson,
        body: jsonEncode(servicio.aJson()),
      );
      return ServicioDto.desdeJson(decodificar(respuesta) as Map<String, dynamic>);
    });
  }

  Future<ServicioDto> reemplazar(int id, ServicioDto servicio) {
    return ejecutar(() async {
      final respuesta = await http.put(
        construirUri('$_ruta/$id'),
        headers: cabecerasJson,
        body: jsonEncode(servicio.aJson()),
      );
      return ServicioDto.desdeJson(decodificar(respuesta) as Map<String, dynamic>);
    });
  }

  Future<void> eliminar(int id) {
    return ejecutar(() async {
      final respuesta = await http.delete(construirUri('$_ruta/$id'));
      decodificar(respuesta);
    });
  }
}
